// ========== CONFIGURAÇÃO ==========
const BASE_URL = '/kant_speak/'; // Ajuste conforme seu servidor
...
async function fetchCurrentItem() {
    try {
        ...
        if (data.success) {
            ...
            showFeedback('❌ Servidor não respondeu corretamente');
        }
    } catch (err) {
...
async function sendAnswer(resposta) {
    ...
    try {
        ...
        if (data.success) {
            ...
            if (isCorrect) {
                // Usa adaptação para próxima atividade
                await getAdaptiveNextActivity();
            }
        } else {
...
async function getAdaptiveNextActivity() {
    try {
        const resp = await fetch(`${BASE_URL}adapt.php`);
        const data = await resp.json();
        if (data.next_activity && data.next_activity !== 'alphabet') {
            // Redireciona para a outra atividade
            window.location.href = `${data.next_activity}.html`;
        } else {
            // Continua na mesma atividade (Alphabet)
            await nextLetter();
        }
    } catch (err) {
        // Fallback: apenas avança letra
        await nextLetter();
    }
}
...
async function init() {
    ...
    // O botão "Próxima letra" ainda pode ser usado manualmente
    nextLetterBtn.addEventListener('click', () => {
        nextLetter();
        logEvent('manual_next', { letter: currentLetter });
    });
